...
function embedBlocChatBot() {
  ...
  const getFloatingMessage = () => {
    ...
    messageEle.style.transition = "opacity 0.5s ease, transform 0.5s ease";
    ...
    window.addEventListener("message", async (event) => {
      const parameter = await event.data;
      ...
      popUpMessageClose.style.backgroundColor =
        COLOR_THEME === "light" ? "#fff" : "#000";
      popUpMessageClose.style.color = COLOR_THEME === "light" ? "#000" : "#fff";
      popUpMessageClose.style.boxShadow =
        COLOR_THEME === "light"
          ? "rgba(150, 150, 150, 0.2) 0px 10px 30px 0px, rgba(150, 150, 150, 0.2) 0px 0px 0px 1px"
          : "rgba(0, 0, 0, 0.2) 0px 10px 30px 0px, rgba(0, 0, 0, 0.2) 0px 0px 0px 1px";
    });
  ...
  function getCloseIcon() {
    const CHAT_BUTTON_CLOSE = `https://embed.askbloc.ai/down.svg`;
